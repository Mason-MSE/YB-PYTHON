...
from cruds.role import get, get_all, create, update, delete
...
def create_item(item_in: RoleCreateSchema, db: Session = Depends(get_db)):
    return create(db, item_in)

@router.put('/{id}', response_model=RoleSchema)
def update_item(id, item_in: RoleUpdateSchema, db: Session = Depends(get_db)):
    db_obj = get(db, id)
    if not db_obj:
        raise HTTPException(status_code=404, detail='Item not found')
